...
import { listNotifications, deleteNotification } from '../api/notifications'
...
export default function AdminDashboard({ user }) {
  ...
  const logs  = useFetch(useCallback(listActivityLogs, []))
  const jr    = useFetch(useCallback(() => orgShortId ? listJoinRequests(orgShortId) : Promise.resolve([]), [orgShortId]))

  // Load overview data on mount
  useEffect(() => {
    inv.load(); qt.load(); cl.load(); notif.load(); jr.load()
  }, [])

  useEffect(() => {
    if (section === 'activity-logs' && !logs.data) logs.load()
  }, [section])

  function signOut() { localStorage.removeItem('token'); navigate('/login') }

  async function handleAccept(reqId) {
    await acceptJoinRequest(orgShortId, reqId)
    jr.load()
  }
  async function handleReject(reqId) {
    await rejectJoinRequest(orgShortId, reqId)
    jr.load()
  }
  ...
  function renderOverview() {
    const stats = [
      ...
      { label: 'Pending Join Requests', value: pendingJR.length, sub: 'awaiting approval', color: '#f59e0b' },
    ]
    return (
      <div className="ds-section">
        ...
        {pendingJR.length > 0 && (
          <div className="ds-alert">
            ...
            <button className="ds-alert-link" onClick={() => setSection('join-requests')}>Review →</button>
          </div>
        )}
      </div>
    )
  ...
  function renderJoinRequests() {
    const rows = jr.data || []
    return (
      <SectionShell title="Join Requests" loading={jr.loading} error={jr.error}
        empty={!rows.length && 'No join requests'}
        actions={<button className="ds-btn-primary" onClick={() => jr.load()}>↻ Refresh</button>}
      >
        <table className="ds-table">
          <thead><tr><th>#</th><th>User ID</th><th>Status</th><th>Requested</th><th>Actions</th></tr></thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id}>
                ...
                <td>
                  <span className="ds-badge" style={{ '--bc': r.status === 'pending' ? '#f59e0b' : r.status === 'accepted' ? '#10b981' : '#ef4444' }}>
                    {r.status}
                  </span>
                </td>
  ...
  function renderActivityLogs() {
    ...
    return (
      <SectionShell title="Activity Logs" loading={logs.loading} error={logs.error}
        empty={!rows.length && 'No activity yet'}
        actions={<button className="ds-btn-primary" onClick={() => logs.load()}>↻ Refresh</button>}
      >
        <table className="ds-table">
          <thead><tr><th>#</th><th>User</th><th>Action</th><th>Resource</th><th>Details</th><th>Time</th></tr></thead>
          <tbody>
            {rows.map(l => (
              <tr key={l.id}>
  ...
  const sections = {
    ...
    'join-requests': renderJoinRequests,
        ...
        <nav className="ds-nav">
          {NAV.map(item => (
            ...
            >
              ...
              {item.label}
              {item.id === 'join-requests' && pendingJR.length > 0 && (
                <span className="ds-nav-badge">{pendingJR.length}</span>
              )}
            </button>
          ))}
        </nav>
